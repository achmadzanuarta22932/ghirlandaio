...
Memasukkan kata sandi untuk autentikasi user cos secara manual.
...
Menampilkan seluruh konfigurasi antarmuka jaringan (IP address, MAC address, status interface) pada server target. Biasanya digunakan untuk memastikan IP ``` 192.168.1.23 ``` sudah terpasang dengan benar.

```
exit
```

Keluar dari sesi Root kembali menjadi user biasa ``` cos ```.

```
exit
```

Menutup koneksi SSH dan keluar dari server ``` 192.168.1.23 ``` kembali ke komputer lokal/master.
...
Keluar dari Root dan mengakhiri sesi SSH di server ``` .23 ```.
...
Kembali masuk ke server ``` .23 ``` karena mendeteksi sesuatu atau ingin memeriksa konfigurasi load balancer bawaan K3s.
